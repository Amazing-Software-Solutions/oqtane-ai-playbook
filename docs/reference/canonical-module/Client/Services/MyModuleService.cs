...
namespace TheCompany.Module.MyModule.Services
{
    ...
    public class MyModuleService : ServiceBase, IMyModuleService
    {
        ...
        {
            List<Models.MyModule> MyModules = await GetJsonAsync<List<Models.MyModule>>(CreateAuthorizationPolicyUrl($"{Apiurl}?moduleid={ModuleId}", EntityNames.Module, ModuleId), Enumerable.Empty<Models.MyModule>().ToList());
